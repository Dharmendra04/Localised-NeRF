...
from localised_nerf.nerf import datasets
from localised_nerf.nerf import models
...
def train_step(model, rng, state, batch, lr):
  ...
  def loss_fn(variables):
    ...
    ret = model.apply(variables,key_0,key_1,rays,supplement, FLAGS.randomized,deterministic=True) #key_0, key_1
...
def main(unused_argv):
  ...
  optimizer = optax.adam(FLAGS.lr_init)
  ...
  def render_fn(variables, key_0, key_1, rays,supplement):
    return jax.lax.all_gather(
        model.apply(variables, key_0, key_1, rays, supplement,FLAGS.randomized,deterministic=False),
        axis_name="batch")
  ...
  init_step = 1
  state = flax.jax_utils.replicate(state) #replicating the state for all devices
  ...
  stats_trace = []
  psnrlist = [] #creating a list to append all epoch psnr values
  reset_timer = True
  for step, batch in zip(range(init_step, FLAGS.max_steps + 1), pdataset):
    ...
    if FLAGS.render_every > 0 and step % FLAGS.render_every == 0:
      ...
      (pred_color, pred_disp, pred_acc, pred_diffuse,
       pred_specular) = utils.render_image(
           functools.partial(render_pfn, eval_variables),
           test_case["rays"],
            test_case['supplement'],
           keys[0],
           FLAGS.dataset == "blender",
           chunk=FLAGS.chunk)
      ...
      if jax.host_id() == 0:
        ...
        summary_writer.image("test_pred_diffuse", pred_diffuse, step)
        summary_writer.image("test_pred_specular", pred_specular, step)
        summary_writer.image("test_target", test_case["pixels"], step)

  if FLAGS.max_steps % FLAGS.save_every != 0:
    state = device_get(jax.tree_map(lambda x: x[0], state))
