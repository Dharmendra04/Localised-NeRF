...
supplement = collections.namedtuple("supplement",('hsv','intensity','cam2world','intrinsics','focal','cam2target'))
...
def define_flags():
  ...
  flags.DEFINE_enum(
      "batching", "single_image", ["single_image"],
  ...
  flags.DEFINE_bool("deterministic", False, "infroming the multi head attention model for inference or training")
  ...
  flags.DEFINE_integer(
      "num_coarse_samples", 96,
      "the number of samples on each ray for the coarse model.")
  flags.DEFINE_integer("num_fine_samples", 96,
...
def render_image(render_fn, rays,supplement, rng, normalize_disp,chunk=8192):
  ...
  for i in range(0, num_rays, chunk):
    ...
    chunk_results = render_fn(key_0, key_1, chunk_rays,supplement)[-1]
    results.append([unshard(x[0], padding) for x in chunk_results])
    # pylint: enable=cell-var-from-loop
  rgb, disp, acc, _, diffuse, specular = [
  ...
  return (rgb.reshape((height, width, -1)), disp.reshape(
      (height, width, -1)), acc.reshape(
          (height, width, -1)), diffuse.reshape(
